...
class ZmaticooAdapterApiTest {

    private val adapterClass = ZmaticooAdapter::class.java
    ...
    @Test
    fun `adapter implements SupportsRegulation interface`() {
        assertThat(SupportsRegulation::class.java.isAssignableFrom(adapterClass)).isTrue()
    }
    ...
    fun `Adapter_Bidding has getToken method`() {
        val getTokenMethod = adapterClass.declaredMethods.find { it.name == "getToken" }
