...
internal class RtbProcessor(
    ...
    /**
     * Load the highest-eCPM RTB payload from cache.
     *
     * Process:
     * 1. Get best payload from RtbPayloadCache (sorted by eCPM descending)
     * 2. Check cancellation before load
     * 3. Create AdSource and load the ad
     * 4. On success: Store in ReadyToShowCache
     * 5. On failure: Remove invalid payload from RtbPayloadCache
     *
    ...
    suspend fun loadBestPayload(
        ...
        // Get best payload from cache (highest eCPM first)
        val payloads = RtbPayloadCache.getAllSortedByEcpm()
        val payload = payloads.firstOrNull()

        if (payload == null) {
            logInfo(TAG, "RTB cache empty, no payload to load")
        ...
        // Check cancellation before starting load
        ensureActive()
        ...
        try {
            ...
            return@coroutineScope when (adEvent) {
                is AdEvent.Fill -> {
                ...
                is AdEvent.LoadFailed, is AdEvent.Expired -> {
                    ...
                    logInfo(TAG, "RTB load failed: demandId=$demandId, removing from cache, error=$error")
                    RtbPayloadCache.remove(demandId)
                    adSource.destroy()
                    ...
                    RtbPayloadCache.remove(demandId)
                    adSource.destroy()
                    Result.failure(BidonError.NoFill(DemandId(demandId)))
                }
            }
            ...
            logInfo(TAG, "RTB load failed: demandId=$demandId, removing from cache, exception=${e.message}")
            RtbPayloadCache.remove(demandId)
            adSource.destroy()
            return@coroutineScope Result.failure(BidonError.NoFill(DemandId(demandId)))
        }
